<html>
<head>
<title>Maintenance Préventive : Porte Coupe-feu </title>
<meta charset="UTF-8" />
...
<body>
	...
	<%
		...
		out.print("<thead><tr><th> N° Porte Coupe-feu </th><th> Emplacement </th><th> Type Porte Coupe-feu </th><th>Observation</th> <th> Etat défecteux </th></tr>");
		out.print("</thead><tbody>");
	      
		for(i=0;i<C.size();i++){
					observation=service.rechercheObservationMaintenanceprev(C.get(i).getNumero());
		...
		conclusion=service.rechercheConclusionMaintenanceprevCoupefeu(num);
        ...
        <table class="table table-bordered table-striped">
          <thead>
            <tr>
             ...
             <th class="col-sm-4">Numéro Porte Coupe-feu</th>
		 ...
		 <tbody>
            ...
            <tr>
              ...
              <td><select name="numCoupefeu" id="liste" class="class_select" v-model="inputCoupefeu">
              <%
              for(i=0;i<C.size();i++){
            	  out.println("<option value=" + C.get(i).getNumero() + ">"
							+ C.get(i).getNumero() + "</option>");
              }
              %>
            </select></td>
		...
		session.setAttribute("organeslist",C);
	%>
	</div>
    ...
    new Vue({
        ...
        methods: {
          ...
          ajouter: function() {
            this.pieces.push({nom: this.inputNom, coupefeu: this.inputCoupefeu});
            $.ajax({
                url: "maintenanceprevcoupefeu.jsp",
                data: {
                	nomPiece: this.inputNom,
                	numCoupefeu: this.inputCoupefeu,
                	etatajout: 'oui'
                },
                });
            <% 
               ajout=request.getParameter("etatajout");
         	   if(ajout!=null){
                   ...
                   piececourante=service.AjoutPiece(nomPiece,numCoupefeu);
